...
async def test_get_user_by_id(user_id, name, surname, age, birthdate, async_client):
...
@pytest.mark.parametrize(
    "name, surname", [("ivan", ""), ("", "ivanov"), ("yakov", ""), ("", "strelkov")]
)
async def test_filter_users_by_name_surname_with_one_empty(name, surname, async_client):
    filter_users = await async_client.get(
        "/users/filter", params={"name": name, "surname": surname}
    )
